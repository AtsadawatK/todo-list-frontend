...
export default function EditTask({ params }) {
  ...
  const [description, setDescription] = useState(
    taskformid ? taskformid.description : ""
  );
    ...
    <div className="flex justify-center h-[100vh] items-center w-full">
      <div className="flex flex-col border  xs:w-[100%] md:w-[75%] lg:w-[60%] xl:w-[45%] xs:h-[100%] md:h-[85%] xs:p-[20px] md:p-[30px]  rounded-xl bg-[#F5F5F7]">
        ...
        <div className="flex flex-col w-[100%] overflow-hidden  h-[100%] py-10">
            ...
            <ThemeProvider theme={theme}>
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  value={dayjs(date)}
                  onChange={(newDate) =>
                    setDate(newDate ? newDate.toISOString() : "")
                  }
                  minDate={dayjs()}
          ...
          <div
            className="my-3 border bg-[#705C53] flex justify-center items-center xs:h-[8%] md:h-[10%] rounded-md xs:text-[16px] lg:text-[18px]  text-[#F5F5F7] font-semibold cursor-pointer"
