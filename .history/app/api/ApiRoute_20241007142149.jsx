...
export async function UpdateTask(id, updateTaskData) {
  try {
    const response = await fetch(
      `https://todo-list-backend-tau.vercel.app/updatetask/${id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
...
export const UpdateCheckedTasks = async (id, isChecked) => {
  const response = await fetch(
    `https://todo-list-backend-tau.vercel.app/updatechecktask/${id}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ _id: id, checked: isChecked }),
    }
  );
...
export function GetTaskfromID(id) {
  ...
  useEffect(() => {
    async function getTaskfromID() {
      const respone = await fetch(
        `https://todo-list-backend-tau.vercel.app/tasks/${id}`
      );
      const data = await respone.json();
      setTaskfromID(data);
    }
...
export async function DeleteTask(id) {
  try {
    const response = await fetch(
      `https://todo-list-backend-tau.vercel.app/deletetask/${id}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
