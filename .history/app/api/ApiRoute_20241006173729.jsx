...
export const addTask = async (newTask) => {
  ...
  const UpdateCheckedTasks = async (checked) => {
    try {
        const response = await fetch('/updatechecktask', {
            method: 'PATCH',
            headers: {
            ...
            body: JSON.stringify({ checked }), // ส่งค่า checked ใน body
        });

        const data = await response.json();

        if (response.ok) {
            console.log(data.message); // "All tasks updated successfully"
        } else {
...
// เรียกใช้ฟังก์ชันนี้ด้วยค่าที่ต้องการ เช่น
updateTasks(true); // อัปเดตให้ checked เป็น true
