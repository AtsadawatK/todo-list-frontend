...
export default function TaskCard() {
  ...
  useEffect(() => {
    ...
    tasks.forEach((task) => {
      initialCheckedState[task._id] = task.checked;
    });
  ...
  const handleCheckTask = (taskId) => {
    setCheckedState((prevState) => ({
      ...prevState,
      [taskId]: !prevState[taskId], // Toggle the checked state
    }));
  };
...
const handleUpdateCheckTask = async () => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      for (const taskId in checkedState) {
        const updateCheck = {
          _id: taskId,
          checked: checkedState[taskId], // ส่งค่า checked ของ task
        };
        await UpdateCheckedTasks(taskId, updateCheck);
      }
...
<div className="text-[18px]">{`You have ${remainingTasksCount} tasks left!`}</div>
        ...
        <div class="flex flex-[5] flex-col w-[100%] overflow-hidden">
          ...
          {tasks.map((task) => {
            const isChecked = checkedState[task._id];
