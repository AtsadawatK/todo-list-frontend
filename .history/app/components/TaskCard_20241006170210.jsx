...
const handleUpdateCheckTask = async () => {
  ...
  if (result.isConfirmed) {
    try {
      // Loop through each task and send updates
      for (const [taskId, checked] of Object.entries(checkedState)) {
        const updateCheck = { checked }; // Create update object for each task
        await UpdateCheckedTasks(taskId, updateCheck); // Call the API for each task
      }
    ...
    router.push('/'); // Redirect after saving changes
  }
};
