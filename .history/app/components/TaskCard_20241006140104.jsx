...
export default function TaskCard() {
  ...
  const [check, setCheck] = useState(tasks);
  ...
  const handleCheckTask = (id) => {
    setCheck(prevTasks => {
      ...
      setHasChanges(true); // ตั้งค่าว่ามีการเปลี่ยนแปลง
      return updatedTasks;
    });
  };
    ...
    const result = await Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const updatedCheck = await UpdateCheckedTasks(id, checkedValue);
          console.log('User updated:', updatedCheck);
        } catch (error) {
...
<div className="text-[18px]">{`You have ${remainingTasksCount} tasks left!`}</div>
        ...
        <div class="flex flex-[5] flex-col w-[100%] overflow-hidden">
          ...
          {tasks.map((task) => (
              <div key={task.id} className={`h-[180px] rounded-xl mr-1 p-3 bg-[#D8D2C2] ${task.checked ? "bg-[#B7B7B7]" : "bg-[#D8D2C2]"}`}>
