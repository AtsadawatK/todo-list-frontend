...
export default function TaskCard() {
  ...
  const [alternateState, setAlternateState] = useState({}); // state สำหรับจัดการ task ที่ checked: false
...
<div className="text-[18px]">{`You have ${remainingTasksCount} tasks left!`}</div>
        ...
        <div class="flex flex-[5] flex-col w-[100%] overflow-hidden">
          ...
          {tasks.map((task) => {
        const isChecked = task.checked ? checkedState[task._id] : alternateState[task._id] checkedState[task._id] || false; // ใช้ alternateState ถ้า checked เป็น false
        return (
