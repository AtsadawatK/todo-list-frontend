...
export default function TaskCard() {
  ...
  const [checkedTasks, setCheckedTasks] = useState(
    tasks.map((task) => ({ ...task, checked: task.checked })) // สร้าง Object ใหม่สำหรับแต่ละ task
  );

  const handleCheckTask = (taskId) => {
    setCheckedTasks((prevCheckedTasks) => {
      const updatedCheckedTasks = [...prevCheckedTasks]; // สร้างสำเนาของ checkedTasks
      const index = updatedCheckedTasks.findIndex((task) => task._id === taskId);
      updatedCheckedTasks[index].checked = !updatedCheckedTasks[index].checked; // Toggle checked state
...
<div className="text-[18px]">{`You have ${remainingTasksCount} tasks left!`}</div>
        ...
        <div class="flex flex-[5] flex-col w-[100%] overflow-hidden">
          ...
          {tasks.map((task, index) => (
              <div key={task,} className={`h-[180px] rounded-xl mr-1 p-3  ${task.checked ? "bg-[#e3e0da]" : "bg-[#D8D2C2]"} ${checkedTasks ? "bg-[#e3e0da]" : "bg-[#D8D2C2]"} `}>
                <div className="flex gap-3  h-[100%]">
                  <div className="flex flex-[0.5] text-[20px]  justify-end items-start">
                  <Checkbox
                  sx={{ padding: 0, paddingTop: "5px" }}
                  checked={checkedTasks[index].checked} // ใช้ checkedTasks[index].checked
