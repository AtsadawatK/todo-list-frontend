  ...
  const result = await Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        ...
        const updatedTask= await UpdateCheckedTasks(id, updateCheck);
...
<div className="text-[18px]">{`You have ${remainingTasksCount} tasks left!`}</div>
            ...
            <div class="flex justify-center items-center w-[40%] h-[100%] bg-[#206e01] hover:bg-[#37b804]  text-[18px]  text-[#F5F5F7] font-semibold rounded-xl cursor-pointer"  onClick={() => handleUpdateTask(task._id)}>
                Save Changes
              </div>
