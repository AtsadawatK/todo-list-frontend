...
export default function AddTask() {
  ...
  const handleAdd = async () => {
    // ถ้า title ว่างให้แสดง error
    if (!isFormValid) return;

    if (!title || !description || !selectedDate) {
      // คุณสามารถเพิ่มการแสดงข้อความ error ที่นี่ได้
      setTitleError(!title); // ตั้งค่า error ถ้า title ว่าง
      setDescriptionError(!description); // ตั้งค่า error ถ้า description ว่าง
      setDateError(!selectedDate); // ตั้งค่า error ถ้า selectedDate ว่าง
      return;
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          // รอ 1500 มิลลิวินาทีก่อนเปลี่ยนเส้นทาง
          setTimeout(() => {
            setTitle('');
            setDescription('');
            setSelectedDate(null);
            setTitleError(false); // แก้ไขเพื่อไม่ให้แสดง error
            setDescriptionError(false); // แก้ไขเพื่อไม่ให้แสดง error
            setDateError(false); // แก้ไขเพื่อไม่ให้แสดง error
            router.push('/'); // เปลี่ยนเส้นทางไปยัง /
          }, 1500);
