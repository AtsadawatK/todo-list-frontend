...
const handleSaveChanges = async () => {
  // สร้าง array เพื่อเก็บการอัปเดต
  const updates = Object.entries(checkedState).map(([taskId, checked]) => ({
    id: taskId,
    checked:,
  }));

  // ทำการเรียก API สำหรับแต่ละการอัปเดต
  try {
    for (const { id, checked } of updates) {
      await UpdateCheckedTasks(id, { checked }); // เรียก API PATCH
    }
...
<div className="text-[18px]">{`You have ${remainingTasksCount} tasks left!`}</div>
            ...
            <div class="flex justify-center items-center w-[40%] h-[100%] bg-[#206e01] hover:bg-[#37b804]  text-[18px]  text-[#F5F5F7] font-semibold rounded-xl cursor-pointer" onClick={handleSaveChanges}>
                Save Changes
              </div>
