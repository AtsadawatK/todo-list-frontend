...
export default function TaskCard() {
  ...
  const [checkedState, setCheckedState] = useState({}); // state สำหรับจัดการ task ที่ checked: true

  const handleCheckTask = (taskId) => {
    setCheckedState((prevState) => ({
      ...prevState,
      [taskId]: !prevState[taskId], // สลับค่า checked ของ task นั้น ๆ
    }));
  };
  const LinkToAddTask = () => {
    router.push('/addtask')
...
<div className="text-[18px]">{`You have ${remainingTasksCount} tasks left!`}</div>
        ...
        <div class="flex flex-[5] flex-col w-[100%] overflow-hidden">
          ...
          {tasks.map((task) => {
        const isChecked = task.checked ? checkedState[task._id] : alternateState[task._id] || false; // ใช้ alternateState ถ้า checked เป็น false
        return (
