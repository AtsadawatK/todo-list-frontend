...
const handleUpdateCheckTask = async () => {
  const result = await Swal.fire({
  ...
  if (result.isConfirmed) {
    try {
      // อัปเดตสถานะของทุก task ที่มีการเปลี่ยนแปลง
      const updatePromises = Object.entries(checkedState).map(async ([taskId, isChecked]) => {
        return await UpdateCheckedTasks({ _id: taskId, checked: isChecked });
      });
      ...
      router.push('/'); // เปลี่ยนเส้นทางหลังจากบันทึกสำเร็จ
    } catch (error) {
      console.error('Update failed:', error);
      Swal.fire("Error!", "Failed to update tasks.", "error");
    }
  }
};
...
<div className="text-[18px]">{`You have ${remainingTasksCount} tasks left!`}</div>
            ...
            <div class="flex justify-center items-center w-[40%] h-[100%] bg-[#206e01] hover:bg-[#37b804]  text-[18px]  text-[#F5F5F7] font-semibold rounded-xl cursor-pointer"  onClick={handleUpdateCheckTask}>
                Save Changes
              </div>
